 ...
 * @return void
 */
function log_write($message, $level = 'ERR', $type = 3, $destination = '', $extra = '')
{
    $now = date('[ c ]');
    if (3 == $type) { // 文件方式记录日志
        if (empty($destination)) {
            // mkdirs(dirname($_SERVER['SCRIPT_FILENAME']) . '/data/logs/');
            // $destination = C('LOG_PATH').date('y_m_d').'.log';
            $destination = dirname($_SERVER['SCRIPT_FILENAME']) . '/data/logs/' . date('y_m_d') . '.log';
